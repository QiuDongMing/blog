...
public class HttpClient {
    ...
    private static ConcurrentHashMap<Integer, RequestConfig> requstConfigCache = new ConcurrentHashMap<Integer, RequestConfig>(
        10);
            ...
            try {
                ...
                if (requestConfig == null) {
                    requestConfig = RequestConfig.custom()
                        .setConnectionRequestTimeout(httpClientConfig
                            .getConnectionRequestTimeout())//该值就是连接不够用的时候等待超时时间，一定要设置，而且不能太大
                        .setConnectTimeout(
                            httpClientConfig.getConnectionTimeout())//连接一个url的连接等待时间（连接超时）
        ...
        if (httpClient == null) {
            ...
            try {
                if (httpClient == null) {
                     ...
                     */
                    connectionManager
                        .setDefaultMaxPerRoute(httpClientConfig.getDefaultMaxPerRoute());
